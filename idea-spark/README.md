...
The coordination model is shared-ledger review, not peer-to-peer chat. A parent creates a room, seeds initial artifacts, launches child agents, waits through bounded discussion phases, records terminal gate decisions, and exports a report from ledger state.
...
├── cli.py / tools.py / store.py
├── resources/                  # bundled skill and references
├── migrations/                 # SQLite schema migrations
├── tests/                      # pytest suite
└── examples/                   # prompt/templates/sample export fixtures
```

No generated caches, local experiment dumps, or version snapshots belong in the tracked project tree.
...
When the package is installed from this source tree as a Python project, the standalone console script `idea-spark` exposes the same subcommands; Hermes plugin installs should rely on `hermes idea-spark ...`.
...
rsync -a --exclude tests --exclude examples --exclude .git --exclude .pytest_cache ./ "$HERMES_HOME/plugins/idea_spark/"
hermes plugins enable idea-spark
```

This is a copy install: later edits in the source tree do not affect the active Hermes plugin until the install command is run again.
...
# Copy the returned room_url to the user only after the dashboard is started/verified; pass dashboard_base_url and check_dashboard=true when you want dashboard_reachable/dashboard_warning in the CLI response.
...
python3 -m pytest tests/test_cli.py tests/test_config.py tests/test_dashboard.py tests/test_examples_contract.py -q
```

## Phase locks

MVP includes the room/message barrier, typed artifact ledger, gates, open needs, fixed Markdown export, protocol examples, default CLI-first operation, explicit config-gated tool-mode, dashboard checks, and plugin discovery checks.
