...
After every successful `idea_spark_room_create`, immediately surface the returned `room_url` to the user, but only present it as an openable live-room link after the dashboard server has been started and the room URL has been health-checked. If a dashboard is already running on a non-default port, pass `dashboard_base_url` in the create payload so the returned link uses the actual URL; pass `check_dashboard=true` to get `dashboard_checked`, `dashboard_reachable`, `dashboard_snapshot_url`, or `dashboard_warning`. Otherwise the default link is `http://127.0.0.1:8765/room/<room_id>`. Do not leave the user with only a `room_id` when the workflow is meant to be a live room, and do not imply that an unverified URL is openable.
...
1. Parent creates one room with `idea_spark_room_create` through CLI default mode or direct tool-mode, then gives the user the returned `room_url` after dashboard reachability is verified or clearly labels it as unverified.
...
8. Use `idea_spark_round_wait` with finite `timeout_s`; exact phase barriers pass `phase`, whole-round barriers omit `phase` or pass `phase="*"`, and multi-phase barriers pass `phases=[...]`. On timeout, continue with partial state and explicitly record missing agents.
...
1. `r0/seed`: Parent creates the room, reports the room-specific dashboard link only after `check_dashboard=true` or another health check proves it reachable, sets `metadata.expected_agents`, and seeds `ResearchGoal`, `IdeaCard`, and `EvaluationRubric` artifacts.
...
```text
1. Write /tmp/is_room.json with title/topic/metadata and, when needed, dashboard_base_url and check_dashboard=true.
2. hermes idea-spark call idea_spark_room_create --json-file /tmp/is_room.json
3. Tell the user the returned room_url only after dashboard_reachable=true or after another health check; if dashboard_warning is present, report the warning and the room id instead of presenting a dead link.
...
7. hermes idea-spark call idea_spark_round_wait --json-file /tmp/is_wait.json with round_id plus optional phase/phase="*"/phases=[...].
...
Operational rule: whenever the parent creates a room, give the user the concrete room URL after verifying it is reachable. If the parent started the dashboard on a random or non-default port, pass that base URL as `dashboard_base_url` to `idea_spark_room_create`; pass `check_dashboard=true` to get `dashboard_reachable` or `dashboard_warning`, and use the returned `room_url` only when it is verified.
