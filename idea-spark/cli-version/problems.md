...
Unresolved product / workflow issue count: 0

Fixed during follow-up: P007, P008, P009, P010, P011, P012. Closed / not-product-defect records from live testing: P001-P006. They are retained below because they were recorded immediately during the run, then resolved by targeted retest, source inspection, or a follow-up patch.

| ID | Status | Severity | Area | Summary |
|---|---|---:|---|---|
| P007 | Fixed in CLI version | Important | `idea_spark_room_status` | `counts.open_needs` now counts unresolved needs only; `counts.total_needs` preserves the all-row count. |
...
| P009 | Fixed in CLI version | Minor | `idea_spark_round_wait` payload ergonomics | `phase` is now optional for whole-round waits; exact phase waits still work. |
| P010 | Fixed in CLI version | Important | `idea_spark_round_wait` barrier semantics | Multi-role barriers can now omit `phase`, use `phase="*"`, or pass `phases=[...]`. |
...
| P012 | Fixed in CLI version | Important | dashboard link availability | `room_create` can now check dashboard reachability with `check_dashboard=true` and return `dashboard_reachable` or `dashboard_warning`. |
...
- CLI-version problems fix targeted RED/GREEN suite: `7 failed` before implementation, then `7 passed in 0.81s`
- CLI-version affected broader suite: `55 passed in 3.22s`
- CLI-version final full suite after P007/P009/P010/P012 fixes: `96 passed in 4.53s`
...
- CLI-version smoke after fixes returned `dashboard_reachable=false` plus `dashboard_warning` for a dead dashboard port, `round_wait` without `phase` returned `phase_filter="*"` and complete arrivals, and resolved needs returned `counts.open_needs=0`, `counts.total_needs=1`

## Fixed issues from live testing

### P007 — fixed in CLI version — `idea_spark_room_status` need count semantics

Status: Fixed in CLI version
...
#### Fix applied in CLI version

1. Added `counts.total_needs` for all rows in the `open_needs` table.
2. Changed `counts.open_needs` to count only unresolved statuses: `open` and `claimed`.
3. Kept `open_need_summary` unchanged.
4. Added regression coverage: `tests/test_tools_room.py::test_room_status_counts_open_needs_as_unresolved_and_total_needs_as_all_rows`.
5. Updated dashboard reader counts to use the same semantics.
...
### P009 — fixed in CLI version — round_wait payload ergonomics
...
**Recommended fix / mitigation:** Fixed in CLI version: `phase` is no longer required for `idea_spark_round_wait`. A parent-style payload with only `room_id`, `round_id`, `expected_agents`, and `timeout_s` now performs a whole-round barrier and returns `phase_filter="*"`. Regression coverage: `tests/test_tools_room.py::test_round_wait_allows_round_only_parent_barrier` and `tests/test_schema_contract.py::test_round_wait_schema_exposes_per_call_expected_agents`.

### P010 — fixed in CLI version — round_wait phase/barrier mismatch in realistic multi-role rounds
...
**Recommended fix / mitigation:** Fixed in CLI version: `idea_spark_round_wait` supports exact `phase`, omitted `phase`, `phase="*"`, and explicit `phases=[...]`. This lets a parent wait for multi-role messages inside one round without forcing every child to use the same phase label. Regression coverage: `tests/test_tools_room.py::test_round_wait_accepts_explicit_multi_phase_barrier`.
...
### P012 — fixed in CLI version — returned room link can be unavailable
...
**Recommended fix / mitigation:** Fixed in CLI version without auto-starting a server: `idea_spark_room_create` accepts `check_dashboard=true` and performs a bounded health check of both `/room/<room_id>` and `/api/rooms/<room_id>/snapshot`. It returns `dashboard_checked=true`, `dashboard_reachable=true`, and `dashboard_snapshot_url` when the link is usable; otherwise it returns `dashboard_reachable=false` plus `dashboard_warning` so the parent can report the warning and room id instead of a dead link. Regression coverage: `tests/test_tools_room.py::test_room_create_can_check_and_warn_when_dashboard_link_is_unreachable` and `tests/test_tools_room.py::test_room_create_can_check_reachable_dashboard_link`.
