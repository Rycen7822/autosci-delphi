#!/usr/bin/env python3
"""Collect second-round live reviewer verdict JSON and record via installed Ponder-Forge CLI.

This is scoped to verifier tasks generated after all 63 gate-gap repair reports were
recorded. It filters by the second-round manifest so old reviewer verdicts cannot be
...
MANIFEST_PATH = ROUND_DIR / "44_rereview_dispatch_manifest.json"
RESULT_DIR = ROUND_DIR / "rereviewer_results"
RECORD_DIR = ROUND_DIR / "rereviewer_record_outputs"
QUEST_PATH = Path("/home/xu/project/loop/DeepScientist/quests/001")
COLLECT_SUMMARY = ROUND_DIR / "46_rereviewer_collect_summary.json"
RECORD_SUMMARY = ROUND_DIR / "46_rereviewer_record_summary.json"
...
def main() -> None:
    ...
    summary: Json = {
        "run_id": RUN_ID,
        "source_verify_output": "42_verify_after_repairs.json",
    ...
    for reviewer_task_id, verdict in sorted(candidates.items(), key=lambda kv: expected[kv[0]]["index"]):
        ...
        path = RESULT_DIR / f"rereviewer_{expected[reviewer_task_id]['index']:02d}_{reviewer_task_id}.json"
    ...
    if args.record:
        if not summary["all_valid"]:
            RECORD_SUMMARY.write_text(json.dumps(summary, indent=2, ensure_ascii=False, sort_keys=True))
    ...
    out = RECORD_SUMMARY if args.record else COLLECT_SUMMARY
