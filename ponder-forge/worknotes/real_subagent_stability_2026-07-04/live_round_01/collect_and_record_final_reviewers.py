#!/usr/bin/env python3
"""Collect final live reviewer verdict JSON and record via installed Ponder-Forge CLI."""
...
MANIFEST_PATH = ROUND_DIR / "56_final_review_dispatch_manifest.json"
RESULT_DIR = ROUND_DIR / "final_reviewer_results"
RECORD_DIR = ROUND_DIR / "final_reviewer_record_outputs"
QUEST_PATH = Path("/home/xu/project/loop/DeepScientist/quests/001")
COLLECT_SUMMARY = ROUND_DIR / "57_final_reviewer_collect_summary.json"
RECORD_SUMMARY = ROUND_DIR / "57_final_reviewer_record_summary.json"
...
def manifest_by_reviewer_task() -> dict[str, Json]:
    ...
    return {str(row["reviewer_task_id"]): row for row in manifest.get("reviewers") or []}
...
def main() -> None:
    ...
    summary: Json = {
        "run_id": RUN_ID,
        "source_gate_output": "53_gate_after_final_repairs.json",
    ...
    for reviewer_task_id, verdict in sorted(candidates.items(), key=lambda kv: expected[kv[0]]["index"]):
        ...
        path = RESULT_DIR / f"final_reviewer_{expected[reviewer_task_id]['index']:02d}_{reviewer_task_id}.json"
